...
if(file.exists(here("processed_data", "m2E_bootstraps.rds")) == TRUE |
  ...
  #if attempting to rerun the bootstrap, update iterations for desired number
  #and use boot_01 rather than mod_boot
  #warning: 1005 iterations will take very long, likely days, to complete
  iterations <- 1
...
if(file.exists(here("processed_data", "seq_data", "h_ci_bootstrap.rds")) == TRUE |
   file.exists(here("processed_data", "seq_data", "f_ci_bootstrap.rds")) == TRUE |
   file.exists(here("processed_data", "seq_data", "u_ci_bootstrap.rds")) == TRUE) {
...
#simulate handling times over temperature range w/ CI
...
#simulate foraging rate over temperature and resource range w/ CI
...
#interpolate length across range of treatment conditions
